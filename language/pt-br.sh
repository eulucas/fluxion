...
FLUXIONStartingScannerTip="Cinco segundos após o AP do alvo aparecer, feche o scanner FLUXION."
...
FLUXIONGeneralXTermFailureError="${CRed}Falha ao iniciar a sessão xterm (configuração possivelmente incorreta)."
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
